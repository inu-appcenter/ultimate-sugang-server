    ...
    inner class 코드값_변환_테스트 {
        ...
        fun 빈_코드는_일요일로_매칭되지_않는다() {
            ...
            assertThatThrownBy { CourseDay.fromCode(blankCode) }
                .isInstanceOf(RestApiException::class.java)
                .hasFieldOrPropertyWithValue("exceptionCode", INVALID_ENUM_TYPE)
            assertThat(CourseDay.SUNDAY.code).isEmpty()
        }
    }

    @Nested
    inner class 정렬_순서_테스트 {
        @Test
        fun 선언_순서가_월요일부터_일요일까지다() {
            //given
            // 시간표 정렬이 enum 선언 순서에 의존한다
            //when & then
            assertThat(CourseDay.entries).containsExactly(
                CourseDay.MONDAY,
                CourseDay.TUESDAY,
                CourseDay.WEDNESDAY,
                CourseDay.THURSDAY,
