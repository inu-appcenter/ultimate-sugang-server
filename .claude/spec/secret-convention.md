...
아래 시크릿은 참조하는 워크플로가 사라졌다.
GitHub 저장소 Settings > Secrets and variables > Actions 에서 삭제한다.

- `RELEASE_DATABASE_URL`: `release` 환경을 없앴다
- `INU_API_BASE_URL`, `INU_API_AUTH_KEY`: 강의 동기화 기능을 없앴다 (#127)
  - **이 변경이 `main`에 배포된 뒤에 지운다.** 그 전의 `main` 워크플로는 아직 이 값을 `inu.course-api`에 주입하고,
    서버는 값이 비면 기동에 실패한다
