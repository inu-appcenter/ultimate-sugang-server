...
interface AuthControllerDocs {
    @Operation(
        summary = "회원가입",
        description = "이메일, 비밀번호와 프로필을 받아 회원을 생성하고 액세스 토큰을 발급합니다.<br>" +
    ...
    @Operation(
        ...
        description = "이메일이 이미 가입에 쓰이고 있는지 확인합니다.<br>" +
            "🔓 <strong>Jwt 불필요</strong><br>"
    )
    @ApiResponses(
    ...
    @Operation(
        summary = "학번 사용 가능 여부 조회",
        description = "학번이 이미 가입에 쓰이고 있는지 확인합니다.<br>" +
            "🔓 <strong>Jwt 불필요</strong><br>"
    )
    @ApiResponses(
    ...
    @Operation(
        summary = "액세스 토큰 재발급",
        description = "만료된 액세스 토큰으로 새 액세스 토큰을 발급받습니다.<br>" +
    ...
    @ApiResponses(
        ...
        ApiResponse(
            responseCode = "401",
            description = "🚨 액세스 토큰 누락 또는 유효하지 않음",
            content = [
                Content(
                    mediaType = MediaType.APPLICATION_JSON_VALUE,
                    examples = [
    ...
    fun reissue(
        @RequestHeader(value = "access-token", required = false) accessToken: String?,
    ): ResponseEntity<AuthTokenResponse>
}
