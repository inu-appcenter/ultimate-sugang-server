...
enum class CourseDepartment(
    ...
    // 교양
    GENERAL_EDUCATION("XAA0", CourseCollege.GENERAL_EDUCATION, "교양", kind = CourseDepartmentKind.NON_MAJOR),

    // 교직
    TEACHING("YAA0", CourseCollege.TEACHING, "교직", kind = CourseDepartmentKind.NON_MAJOR),

    // 일선
    GENERAL_ELECTIVE("WAA0", CourseCollege.GENERAL_ELECTIVE, "일선", kind = CourseDepartmentKind.NON_MAJOR),

    // 군사학
    MILITARY("ZAA0", CourseCollege.MILITARY, "군사학", kind = CourseDepartmentKind.NON_MAJOR),
    ...
    // 연계전공
    INU_LIBERAL_ARTS("", CourseCollege.ETC, "INU리버럴아츠연계전공", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    MICE_SPORTS_TOURISM("", CourseCollege.ETC, "MICE,스포츠및관광연계전공", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    PERFORMING_VISUAL_ARTS("", CourseCollege.ETC, "공연예술과시각예술연계전공", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    PUBLIC_HEALTH("", CourseCollege.ETC, "공중보건연계전공", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    OPTOELECTRONICS("VAB1", CourseCollege.ETC, "광전자공학전공(연계)", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    INTERNATIONAL_DEVELOPMENT_COOPERATION("", CourseCollege.ETC, "국제개발협력연계전공", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    INTERNATIONAL_BUSINESS_TAX("", CourseCollege.ETC, "국제비즈니스및세무연계전공", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    GLOBAL_ENTREPRENEURSHIP("", CourseCollege.ETC, "글로벌기업가정신연계전공", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    CLIMATE_ENERGY_ENVIRONMENT("", CourseCollege.ETC, "기후,에너지및환경연계전공", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    GREEN_CLIMATE("", CourseCollege.ETC, "녹색기후연계전공", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    GREEN_CITY("", CourseCollege.ETC, "녹색도시연계전공", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    NORTHEAST_ASIAN_STUDIES("", CourseCollege.ETC, "동북아지역학전공(연계)", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    LOGISTICS("VAC1", CourseCollege.ETC, "물류학전공(연계)", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    FUTURE_EDUCATION_DESIGN("0000849", CourseCollege.ETC, "미래교육디자인연계전공", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    FUTURE_CITY("", CourseCollege.ETC, "미래도시연계전공", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    FUTURE_AUTOMOBILE("0000789", CourseCollege.ETC, "미래자동차연계전공", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    BIO_CONVERGENCE_STARTUP("", CourseCollege.ETC, "바이오융합\u00B7창업연계전공", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    BEAUTY_INDUSTRY("", CourseCollege.ETC, "뷰티산업연계전공", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    SOCIAL_DATA_SCIENCE("0000678", CourseCollege.ETC, "소셜데이터사이언스연계전공", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    RENEWABLE_ENERGY("", CourseCollege.ETC, "신재생에너지연계전공", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    EUROPEAN_TRADE("", CourseCollege.ETC, "유럽통상학전공(연계)", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    GENOMICS("", CourseCollege.ETC, "유전체학연계전공", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    AI_STARTUP("", CourseCollege.ETC, "인공지능\u00B7창업연계전공", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    AI_SOFTWARE("", CourseCollege.ETC, "인공지능소프트웨어연계전공", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    HUMANITIES_CULTURE_ART_PLANNING("0000677", CourseCollege.ETC, "인문문화예술기획연계전공", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    STEM_CELL_TISSUE_ENGINEERING("", CourseCollege.ETC, "줄기세포및조직공학연계전공", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    CHINA_STUDIES("", CourseCollege.ETC, "중국연구연계전공", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    CHINESE_REGIONAL_STUDIES("", CourseCollege.ETC, "중국지역학전공(연계)", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    INTELLIGENT_ROBOT("", CourseCollege.ETC, "지능로봇연계전공", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    INTELLIGENT_ROBOT_SYSTEM("0000912", CourseCollege.ETC, "지능형로봇시스템연계전공", kind = CourseDepartmentKind.INTERDISCIPLINARY),
    ...
    companion object {
        ...
        fun fromDepartment(department: String): CourseDepartment {
            ...
            if (!courseDepartment.isDepartment()) {
                throw RestApiException(INVALID_DEPARTMENT)
            }
        ...
        fun fromInterdisciplinary(department: String): CourseDepartment {
            ...
            if (!courseDepartment.isInterdisciplinary()) {
                throw RestApiException(INVALID_INTERDISCIPLINARY_DEPARTMENT)
            }
