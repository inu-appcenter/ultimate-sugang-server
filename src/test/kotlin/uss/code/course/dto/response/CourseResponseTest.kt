...
class CourseResponseTest(
    ...
    private fun createCourseWithSchedule(): Course {
    ...
    inner class 강의_식별자_매핑_테스트 {
        ...
        fun 식별자는_문자열로_내려간다() {
            ...
            //then
            assertThat(response.id).isEqualTo(course.id.toString())
        }

        @Test
        fun 학수번호는_code_에_과목코드는_courseCode_에_담긴다() {
            //given
            val course = createCourse()
        ...
        @Test
        fun 국문명과_영문명이_각각_name_과_nameEn_에_담긴다() {
            //given
            val course = createCourse()
    ...
    inner class 값이_없는_필드_테스트 {
        ...
        @Test
        fun 시간표가_있으면_요일_교시_강의실_표기로_담긴다() {
            //given
            val course = createCourseWithSchedule()
    ...
    @Nested
    inner class 이수영역_매핑_테스트 {
        @Test
        fun 교양_영역이면_이수영역_명칭이_담긴다() {
            //given
            val course = courseRepository.save(
                CourseFixture.createCourse(
    ...
    @Nested
    inner class 태그_조립_테스트 {
        @Test
        fun 태그_대상이_아니면_빈_목록이다() {
            //given
            val course = createCourse()
        ...
        @Test
        fun 교시_코드가_B로_시작하면_긴_수업_태그가_붙는다() {
            //given
            val course = createCourseWithSchedule()
    ...
    @Nested
    inner class 야간학과_표기_테스트 {
        @Test
        fun 학과명이_야로_끝나면_야간학과다() {
            //given
            val course = courseRepository.save(
    ...
    @Nested
    inner class 마감_여부_테스트 {
        @Test
        fun 정원이_남아_있으면_마감이_아니다() {
            //given
            val course = createCourse()
